...
> - All LLM rows/tables in this report currently refer to **OpenAI `gpt-4o-mini`** runs.
> - **Gemma4** experiments are ongoing and their results are **not included yet**.
> - Add Gemma4 as a separately labeled model block once outputs are finalized.
...
**Related:** `docs/LLM_EXPERIMENT_REPORT_GPT4O_MINI.md` (LLM-only deep dive), `METHODOLOGY.md` (pipeline).
