...
> - All numerical results currently reported in this document are from **OpenAI `gpt-4o-mini`** runs.
> - **Gemma4** experiment runs are still in progress and their results are **not included yet**.
> - When Gemma4 results are available, report them in separately labeled sections/tables to avoid model-mixing.
...
- The model predicts whether the **immediate next hospital admission** for the same patient will include a lipid-disorder diagnosis code (`272.x`, stored without dot in MIMIC-III).
- This is a prediction of **coded diagnosis occurrence on the next admission**, not a direct estimate of true underlying dyslipidemia burden or long-term cardiovascular risk.
- The target is constructed from `diagnoses_codes_next` in adjacent visit pairs (current visit -> next visit).

### Is the Horizon Next Day / Week / Month?

- The pipeline does **not** define a fixed temporal horizon (for example, 7-day, 30-day, or 90-day prediction).
- Horizon is **event-based**: the next recorded admission in chronological order for that patient.
- Therefore, the elapsed time between current and next visit varies by patient.
- Patients without a subsequent admission are not represented as labeled pairs for this task.

### Observed Data Distribution and Class Balance

From `data/outputs/mimiciii_gpt4o_mini/`:
...
- The dataset is **imbalanced** (about **72.6% negative** and **27.4% positive**).
- This is a **moderate imbalance**; stratified splitting kept class proportions nearly identical across train and test.
...
## 10. Results (Archived Full Run — `gpt-4o-mini`)

The following values are taken from **`data/outputs/mimiciii/summary_table.csv`** and **`llm_*_metrics.json`** as they exist in the repository after a completed full pipeline run. These values correspond to **`gpt-4o-mini`** and summarize performance on **n = 2,492** test pairs unless noted.
